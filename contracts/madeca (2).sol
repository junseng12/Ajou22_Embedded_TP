...
contract test{
    ...
    enum GameStatus {Open, Settled, OnAir, Closed}
    ...
    function makeGame(uint256 _gameId, uint256 _startAt, uint256 _finishAt, uint _joinFeeAmount, uint _betFeeAmount) public payable isMember{
    ...
    //penalty
    function cancelGame(uint256 _gameId) public isMaker(_gameId) gameOpen(_gameId){
        ...
        for(uint i = 0; i < 10; i++){
            if(gamePlayer[uint64(_gameId)][i] != address(0))
                payable(gamePlayer[uint64(_gameId)][i]).transfer(_joinFeeAmount);
        }
        // bettor에게 수수료 제외한 betFee 돌려주기
        uint _betFeeAmount = calcCut(gameList[uint64(_gameId)].betFeeAmount);
        for(uint i = 0; i < betTeam1[uint64(_gameId)].length; i++){         //만약 push를 쓴다고 하면 반대로 length는 쓰면 안됨
    ...
    function joinGame(uint256 _gameId, bool team) public payable gameOpen(_gameId) isMember{
    ...
    //penalty
    function exitGame(uint256 _gameId) public isPlayer(_gameId) gameOpen(_gameId){
    ...
    function betting(uint256 _gameId, bool team, uint n) public payable canBetting(_gameId) gameSettled(_gameId) isMember{
    ...
    function exitBetting(uint256 _gameId) public canBetting(_gameId) gameSettled(_gameId) isMember{
    ...
    function closeJoin(uint256 _gameId) public isMaker(_gameId) gameOpen(_gameId){
        //참가자 10명이 채워져야 종료가능
    ...
    function startGame(uint256 _gameId) public isMaker(_gameId) gameSettled(_gameId){
    ...
    function finishGame(uint256 _gameId) public isMaker(_gameId) gameOnAir(_gameId){
    ...
    function giveBet(uint _gameId) private {
        ...
        if(gameList[uint64(_gameId)].winner){
            for(uint i = 0; i < numberOfWinner; i++){
                payable(betTeam1[/*winner*/uint64(_gameId)][i]).transfer(perBetFee);
            }
        }
        else{
            for(uint i = 0; i < numberOfWinner; i++){
                payable(betTeam2[/*winner*/uint64(_gameId)][i]).transfer(perBetFee);
            }
        }
    }
    ...
    //}
    function calcCut(uint _fee) view private returns(uint){
        return _fee - cut;
    }

    function searchPlayerIndex(address player, uint256 _gameId) view private returns(uint){
        for(uint i = 0; i < 10; i++){
    ...
    function searchBettorIndex(address bettor, uint256 _gameId) view private returns(uint){
    ...
    function searchUser(address user) view private returns(bool){
    ...
    function searchPlayerNumber(uint256 _gameId) view private returns(uint){
        uint num = 0;
        for(uint i = 0; i < 10; i++){
            if(gamePlayer[uint64(_gameId)][i] != address(0)){
                num += 1;
